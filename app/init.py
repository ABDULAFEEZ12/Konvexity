...
def configure_error_handlers(app):
    def render_error(status_code, message, description=None):
        template_map = {404: "errors/404.html", 500: "errors/500.html"}
        template = template_map.get(status_code, "errors/500.html")
        return render_template(template, status_code=status_code, message=message, description=description), status_code
    ...
    def not_found(error):
        return render_error(404, "Page Not Found", "The page you are looking for does not exist or has been moved.")

    @app.errorhandler(405)
    def method_not_allowed(error):
        return render_error(405, "Method Not Allowed", "The HTTP method used is not supported for this resource.")

    @app.errorhandler(429)
    def too_many_requests(error):
        return render_error(429, "Too Many Requests", "You have made too many requests. Please wait before trying again.")

    @app.errorhandler(500)
    def internal_server_error(error):
        logger.exception("Internal server error: %s", str(error))
        return render_error(500, "Internal Server Error", "An unexpected error occurred. Our team has been notified.")
    ...
    def inject_seo_defaults():
        return {
            "seo": {
                ...
                "description": (
                    "Konvexity is a premium leadership, organizational transformation, and human performance "
                    "consulting company helping founders, executives, and institutions achieve sustainable growth."
                ),
                "canonical_url": None,
                "og_type": "website",
                "og_image": "/static/images/brand/konvexity-og.jpg",
            }
        }
